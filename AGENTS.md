...
## Testing Guidelines
- Parcours complet : configurer `PAYPAL_CLIENT_ID`/`PAYPAL_SECRET` sandbox + `BREVO_API_KEY`/`FROM_EMAIL` + credentials Supabase (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`), démarrer le serveur puis réaliser un achat via `http://localhost:4000/purchase.html`. Contrôler l’e-mail reçu et scanner le QR généré.
